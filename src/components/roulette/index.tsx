...
export const RouletteWrapper = styled.div<Wrapper>`
  ...
  // background: linear-gradient(
    ...
    rgb(255 255 255) 67.71%,
    rgb(142 142 142) 87.5%
  );
  // background: ${(props) =>
    props.picked
...
class Roulette extends React.Component<IProps, IState> {
  ...
  renderItem = (height: number) => (item: IRouletteItem, index: number) =>
    (
      <RouletteItem
        key={`${item.id} ${index}`}
        item={item}
  ...
  render() {
    ...
    return (
      <RouletteWrapper height={rouletteHeight} picked={picked}>
        ...
        <Gradient
          onClick={() =>
            this.props.canPick ? this.props.onPick && this.props.onPick() : {}
          }
        />
      </RouletteWrapper>
    );
